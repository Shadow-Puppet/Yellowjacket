...
dev: setup generate clean
	set -a && [ -f .env ] && . .env; set +a; go tool wails dev -tags webkit2_41 -loglevel Debug -v 2

dev-debug: setup generate clean
	set -a && [ -f .env ] && . .env; set +a; YJ_LOG_LEVEL=debug go tool wails dev -tags webkit2_41 -loglevel Debug -v 2
